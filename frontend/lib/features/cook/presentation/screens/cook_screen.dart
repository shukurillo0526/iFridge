...
class _CookScreenState extends State<CookScreen>
  ...
  void _openAiGenerate(BuildContext context) async {
    // Get user's inventory ingredient names for AI recipe generation
    final client = Supabase.instance.client;
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Generating a recipe from your ${ingredientNames.length} ingredients...',
    ...
    try {
      ...
      final result = await api.generateRecipe(ingredients: ingredientNames);
      api.dispose();
      ...
      final steps = (data['steps'] as List?)?.map((s) => s['text'] ?? '').join('\n') ?? 'No steps.';
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          title: Row(
            children: [
              const Icon(Icons.auto_awesome, color: IFridgeTheme.primary, size: 20),
          ...
          content: SingleChildScrollView(
            child: Text(steps, style: const TextStyle(color: Colors.white70, height: 1.6)),
          ),
          actions: [
            TextButton(
